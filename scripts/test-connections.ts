 ...
 * Tests Supabase, MongoDB, and Qdrant connections
 */
...
async function testMongoDB() {
  ...
  try {
    ...
    const hostInfo = mongoUri.includes("@")
      ? mongoUri.split("@")[1]
      : "MongoDB";
    ...
    if (error.message.includes("Authentication failed")) {
      ...
      log(
        "   Check the hostname/IP address in your connection string",
        colors.yellow,
      );
      ...
      log(
        "   2. Check if MongoDB is listening on the correct port (default: 27017)",
        colors.yellow,
      );
      log(
      ...
      log(
        "   1. Check EC2 security group allows inbound on port 27017",
        colors.yellow,
      );
      log(
        "   2. Check MongoDB bind_ip in /etc/mongod.conf (should be 0.0.0.0 for remote access)",
        colors.yellow,
      );
      log(
        "   3. Check if UFW firewall is blocking: sudo ufw status",
        colors.yellow,
      );
    } else if (error.message.includes("IP")) {
      log("\n💡 Tip: IP whitelist issue", colors.yellow);
      log(
        "   For self-hosted MongoDB, check firewall rules instead of IP whitelist",
        colors.yellow,
      );
    }
...
async function testQdrant() {
  ...
  try {
    ...
    log(
      `   Available collections: ${collections.collections?.map((c) => c.name).join(", ") || "None"}`,
      colors.green,
    );
    ...
    if (collectionExists) {
      ...
      log(
        `   Distance metric: ${info.config?.params?.vectors?.distance || "N/A"}`,
        colors.green,
      );
    } else {
      log(`   ⚠️  Collection "${collectionName}" not found`, colors.yellow);
      log(
        "   The collection will be created automatically on first vector upload",
        colors.yellow,
      );
    ...
    if (error.message.includes("ECONNREFUSED")) {
      log(
        "\n💡 Tip: Qdrant server is not running or not accessible",
        colors.yellow,
      );
      log("   1. Check if Qdrant is running on EC2", colors.yellow);
      log(
        "   2. Verify EC2 security group allows inbound on port 6333",
        colors.yellow,
      );
      ...
      log("\n💡 Tip: Connection timeout", colors.yellow);
      log(
        "   1. Check EC2 security group allows inbound on port 6333",
        colors.yellow,
      );
...
async function runAllTests() {
  ...
  log(
    `Qdrant (Vector DB): ${results.qdrant ? "✅ Connected" : "❌ Failed"}`,
    results.qdrant ? colors.green : colors.red,
  );
